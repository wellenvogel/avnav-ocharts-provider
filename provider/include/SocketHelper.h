...
public:
    static bool LogSysError(int res,const char *txt,const char *ip,int port){
        ...
        LOG_ERROR(wxT("%s(%s:%d): SYSERROR: %d=%s"),txt,ip,port,errno,(ep?ep:"null"));
        return true;
    }
    static bool LogSysError(int res,const char *txt,int fd){
        ...
        char buffer[200];
        const char *ep=strerror_r(errno,buffer,200);
        LOG_ERROR(wxT("%s(fd=%d): SYSERROR: %d=%s"),txt,fd,errno,(ep?ep:""));
        return true;
    }
    ...
    static wxString GetAddress(SocketAddress sa ){
    ...
    static int WriteAll(int socket,const void * buffer, int len, long timeout=0){
        ...
        while (remain > 0 &&(timeout == 0 || (current >= start && current <=end))){
            current=wxGetLocalTimeMillis();
            int wr=Write(socket,bp+offset,remain,(timeout == 0)?0:(end-current).ToLong());
